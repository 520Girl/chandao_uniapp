    ...
    <view class="flex-1 px-4 py-6 space-y-8">
      ...
      <view class="mb-10 overflow-hidden">
        ...
        <view class="flex items-center gap-4 overflow-x-auto pb-[32rpx] no-scrollbar">
          <view v-for="(item, index) in rankList" :key="index" class="flex flex-col items-center gap-2">
            <view
              :class="index === 0 ? 'border-2 border-theme-1 shadow-[0_0_15px_rgba(212,175,53,0.3)]' : 'border border-primary/20'"
              class="relative p-[12rpx] rounded-full">
              <image alt="User" class="w-[90rpx] h-[90rpx] rounded-full object-cover" :src="item.imgUrl"
                mode="aspectFill" />
            </view>
            <span :class="index === 0 ? 'theme-color-1' : 'theme-color-8'" class="text-[20rpx] font-bold ">{{ item.name
            }}</span>
          </view>
        </view>
      </view>
      <view class="space-y-6">
        <view v-for="(item, index) in communityList" :key="index"
          ...
          class="relative border border-[#d4af35]/5 p-6 rounded-[2rem]  mb-6">
          <view class="flex items-center gap-3 mb-6">
            ...
            <view>
              ...
              <view class="flex items-center gap-1">
                ...
                <template v-if="item.type === 'active'">
                  <text class="iconfont icon-MapPin text-[20rpx] cloud-glow-active theme-color-8"></text>
                  <text class="text-[20rpx] theme-color-8 font-medium uppercase tracking-wider block">{{ item.address
                }}</text>
                  <text class="text-[80rpx] theme-color-2 iconfont italic icon-huodong text-[#b4d2e761] absolute -right-0 -top-0"></text>
                </template>
                <template v-else>
                  <text class="text-[20rpx] theme-color-8 font-medium uppercase tracking-wider block">{{ item.desc
                }}</text>
            ...
            <text class="iconfont text-[28rpx] theme-color-1 ml-auto" :class="'icon-' + item.Mood"></text>

          </view>
          <template v-if="item.type === 'active'">
            <text class="theme-color-7 leading-relaxed mb-[32rpx] italic text-[30rpx] px-[16rpx] block first-letter:text-3xl first-letter:mr-3 first-letter:float-left first-letter:font-headline first-letter:text-primary/80"
            @click="onNavigate(item)">“{{ item.desc }}”</text>
          </template>
          <template v-else>
            <text class="theme-color-7 leading-relaxed mb-[32rpx] italic text-[30rpx] px-[16rpx] block "
            @click="onNavigate(item)">“今日照见：<text class="theme-color-1">躁动</text>的情绪如云掠过静止的湖面，未曾触及水面分毫。”</text>
          </template>
         
          <view class="grid grid-cols-2 gap-2 mb-[32rpx]">
            <image class="w-full h-[256rpx] object-cover rounded-[30rpx]" v-for="(img, index) in item.imgUrl" :key="index"
              data-alt="Minimalist modern interior design with natural light" :src="img" />
          </view>
          <view class="flex items-center justify-between border-t border-[#d4af35]/5 pt-1">
            <view class="flex items-center gap-6" v-if="item.type === 'post'">
              <view class="flex items-center gap-1 " :class="index % 2 === 1 ? 'theme-color-1' : 'theme-color-12'">
                ...
                <text class="text-[24rpx] font-bold ">{{ item.heart }} 点亮</text>
              </view>
              <view class="flex items-center gap-1 theme-color-8">
                <text class="iconfont icon-chat-bubble-1 text-[35rpx]"></text>
                <text class="text-[24rpx] font-medium">{{ item.comment }} 回响</text>
              </view>
            </view>
            <view class="flex items-center gap-1" v-if="item.type === 'active'">
              <view
                  class="flex items-center gap-1 theme-color-1">
                  <text class="iconfont icon-canjia text-[24rpx]"></text>
                  <text class="text-[24rpx] font-medium ">{{ item.inJoin }}</text>
                </view>
              <view class="flex items-center ml-[20rpx] px-[24rpx] text-[24rpx] py-[20rpx] rounded-full bg-theme-13 theme-color-1 text-xs font-bold transition-transform active:scale-95" v-if="item.inJoinStatus === 'false'">
                <text class="iconfont icon-person-fill text-[35rpx]"></text>
                <text class="text-[24rpx] icon-done-all iconfont font-medium">申请加入</text>
              </view>
            </view>
...
const communityList = ref([
  { name: '静心禅修', Mood: 'Raining', nickname: "校长", desc: "静坐24分钟后", type: 'active', members: 1200, time: '2m ago', inJoin: 100, inJoinStatus: "true", imgUrl: '',startTime: '2026-03-28 10:00:00',endTime: '2026-03-28 10:30:00',address: '北京海淀区'},
  { name: '晨间觉察', Mood: 'Sunny', nickname: "副校长", desc: "晨间觉察10分钟后", type: 'active', members: 850, time: '2m ago', inJoin: 100, inJoinStatus: "false", imgUrl: '',startTime: '2026-03-28 10:00:00',endTime: '2026-03-28 10:30:00',address: '北京海淀区2'},
  { name: '夜晚反思', Mood: 'Cloudy', nickname: "助理", desc: "夜晚反思15分钟后", type: 'post', members: 600, time: '2m ago', heart: 100, comment: 20, imgUrl: ['https://picsum.photos/seed/community2/200/200','https://picsum.photos/seed/community2/200/200'] },
]);
